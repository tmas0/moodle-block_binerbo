...
function addParentPath(node,breadcrumb_ul) {
    if (firstNonEmptyChild(firstNonEmptyChild(node)) &&
        (firstNonEmptyChild(node).tagName == 'DIV' || firstNonEmptyChild(node).tagName == 'A')) {
        var text;
        var divNode;
...
function expandParent(node) {
    if ( !elementHasClass(node,menu_class) ) {
        if ( firstNonEmptyChild(node) &&
            firstNonEmptyChild(firstNonEmptyChild(node)) &&
            firstNonEmptyChild(firstNonEmptyChild(node)).parentNode) {
...
function firstNonEmptyChild(element) {
    if (element.firstChild &&
        (element.firstChild.nodeType != TEXT_NODE_TYPE ||
            (element.firstChild.nodeValue &&
                element.firstChild.nodeValue.replace(/^\s*|\s*$/g,"") != '')) ) {
