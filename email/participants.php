...
if ( $CFG->email_old_select_participants ) {
    ...
    foreach ($alpha as $ch) {
        ...
        if ($ch == $firstinitial) {
            ...
            echo "<a href=\"$CFG->wwwroot/blocks/email_list/email/participants.php
                    ?id=$courseid&amp;group=$selgroup&amp;perpage=$perpage&amp;
                    search=$search&amp;roleid=$roleid&amp;lname=$lastinitial&amp;fname=$ch')\">$ch</a>";
        }
        $nch++;
    }
    echo '</li>';
    ...
    foreach ($alpha as $ch) {
        ...
        if ($ch == $lastinitial) {
            ...
            echo "<a href=\"$CFG->wwwroot/blocks/email_list/email/participants.php?
                    id=$courseid&amp;group=$selgroup&amp;perpage=$perpage
                    &amp;search=$search&amp;roleid=$roleid&amp;fname=$firstinitial&amp;lname=$ch')\">$ch</a>";
        }
        $nch++;
    }
    ...
    echo '<li>';
    echo '<a href="' . $CFG->wwwroot . '/blocks/email_list/email/participants.php?id=' . $courseid .
            '&amp;group=' . $selgroup . '">' . get_string('allusersincourse', 'block_email_list') . '</a>';
    echo '</li></ul></div>';
    ...
    if ( count($rolenames) > 1 ) {
        ...
        popup_form("$CFG->wwwroot/blocks/email_list/email/participants.php
                ?id=$courseid&amp;group=$selgroup&amp;page=$page&amp;perpage=$perpage
                &amp;search=$search&amp;fname=$firstinitial&amp;
                lname=$lastinitial&amp;contextid=$context->id&amp;roleid=",
            $rolenames,
            'rolesform',
            $roleid,
            ''
    ...
    echo '<div id="participants"></div>' .
        '<iframe id="idsearch" name="bssearch" src="get_users.php?id=' . $courseid .
        '&amp;roleid=' . $roleid . '&amp;group=' . $selgroup . '&amp;page=' . $page .
        '&amp;perpage=' . $perpage . '&amp;search=' . $search . '&amp;fname=' . $firstinitial .
        '&amp;lname=' . $lastinitial . '" style="display:none;"></iframe>' . "\n\n";

    print_spacer(1, 4, false);
    if ( $perpage == '7' ) {
        echo '<div id="to_all_users" class="all_users"><img src="' . $CFG->wwwroot .
            '/blocks/email_list/email/images/add.png" height="16" width="16" alt="' . get_string("course") .
            '" /> <a href="' . $CFG->wwwroot . '/blocks/email_list/email/participants.php?id=' . $courseid .
            '&amp;group=' . $selgroup . '&amp;perpage=99999&amp;search=' . $search . '&amp;roleid=' . $roleid .
            '&amp;fname=' . $firstinitial . '&amp;lname=' . $lastinitial . '">' . get_string('showallusers') .
            '</a></div>';
    } else {
        echo '<div id="to_all_users" class="all_users"><img src="' . $CFG->wwwroot .
            '/blocks/email_list/email/images/delete.png" height="16" width="16" alt="' . get_string("course") .
            '" /> <a href="' . $CFG->wwwroot . '/blocks/email_list/email/participants.php?id=' . $courseid .
            '&amp;group=' . $selgroup . '&amp;perpage=7&amp;search=' . $search .
            '&amp;roleid=' . $roleid . '&amp;fname=' . $firstinitial . '&amp;lname=' . $lastinitial .
            '">' . get_string('showperpage', '', 7) . '</a></div>';
    }
}
